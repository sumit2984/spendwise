...
//Today Expense
$tdate=date('Y-m-d');
$query=mysqli_query($con,"select sum(ExpenseCost)  as todaysexpense from tblexpense where (ExpenseDate)='$tdate'");
...
$query1=mysqli_query($con,"select sum(ExpenseCost)  as yesterdayexpense from tblexpense where (ExpenseDate)='$ydate'");
...
$crrntdte=date("Y-m-d");
$query2=mysqli_query($con,"select sum(ExpenseCost)  as weeklyexpense from tblexpense where ((ExpenseDate) between '$pastdate' and '$crrntdte')");
...
$crrntdte=date("Y-m-d");
$query3=mysqli_query($con,"select sum(ExpenseCost)  as monthlyexpense from tblexpense where ((ExpenseDate) between '$monthdate' and '$crrntdte')");
...
//Yearly Expense
 $cyear= date("Y");
$query4=mysqli_query($con,"select sum(ExpenseCost)  as yearlyexpense from tblexpense where (year(ExpenseDate)='$cyear')");
...
$query6=mysqli_query($con,"select count(id) as toalcategory from tblcategory");
$result6=mysqli_fetch_array($query6);
$totalcategories=$result6['toalcategory'];
 ?>
					<div class="panel-body easypiechart-panel">
						<h4>Total Listed Categories</h4>
						<a href="manage-category.php">
				<div class="easypiechart" id="easypiechart-blue" data-percent="100" ><span class="percent"><?php 
echo $totalcategories;?></span>
...
$totalusers=$result7['totalusers'];
 ?>
				...
				<div class="easypiechart" id="easypiechart-teal" data-percent="100" ><span class="percent"><?php 
echo $totalusers;?></span>
